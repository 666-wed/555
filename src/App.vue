...
async function submitLogin() {
  if (authLoading.value || !authForm.value.code) return
  ...
  <div v-if="!isAuthenticated" class="login-shell">
    ...
    <section class="login-form-side">
      <div class="login-form-wrap">
        ...
        <h2>欢迎回来</h2>
        <p class="login-subtitle">登录后继续记录你们正在发生的故事。</p>
        <form class="login-form" @submit.prevent="submitLogin">
          ...
          <div class="input-wrap code-input-wrap">
            <LockKeyhole :size="17" />
            <input id="login-code" v-model="authForm.code" inputmode="numeric" maxlength="6" autocomplete="one-time-code" placeholder="输入 6 位验证码" required />
            <button class="code-button" type="button" :disabled="codeSending || codeCountdown > 0" @click="requestCode">
              {{ codeSending ? '发送中…' : codeCountdown > 0 ? `${codeCountdown}s 后重发` : '获取验证码' }}
            </button>
          </div>
          <div v-if="authError" class="auth-error">{{ authError }}</div>
          <div v-if="codeMessage" class="code-message">{{ codeMessage }}</div>
          <button class="login-button" type="submit" :disabled="authLoading">
            <LogIn :size="17" />
            {{ authLoading ? '正在进入…' : '进入家庭空间' }}
          </button>
        </form>
        <div class="login-hint"><Mail :size="14" /><span>验证码将发送到你的真实邮箱，10 分钟内有效。</span></div>
